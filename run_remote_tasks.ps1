...
if (Test-Path "$cwd\processes.ps1"){}
else{
	write-host "Missing gatherhashes.ps1 from working directory: Exiting"
	Stop-Transcript
	exit
}
	...
	if ($hostname -ne "") {
		...
		if ($hostname -like '*-*-*' ) { }
			...
			while ($sleepcount -lt 30)    {
				...
				if ($results)     {
					write-host "$results`n" -foregroundcolor "gray"
				}
...
$results = Get-Job | Receive-Job 2>&1write-host "`n$results" -foregroundcolor "darkblue"
...
## Continually re-run until no hosts are skipped
# if ($skipped -eq "True"){
#       Start-Sleep 900
#       $cwd/Scan_systems.ps1 
# }
# exit
